...
def _establishment_sources(payload: dict) -> list[tuple[str, bool]]:
    singular_paths = [payload["source_path"]] if payload.get("source_path") else []
    singular_urls = [payload["source_url"]] if payload.get("source_url") else []
    plural_paths = payload.get("source_paths") or []
    plural_urls = payload.get("source_urls") or []
    if not isinstance(plural_paths, list) or not isinstance(plural_urls, list):
        raise ValueError("source_paths and source_urls must be lists")
    groups = [
        group
        for group in (singular_paths, singular_urls, plural_paths, plural_urls)
        if group
    ]
    if len(groups) != 1:
        raise ValueError("Provide exactly one establishment source path or URL collection")
    values = groups[0]
    if len(values) > 10:
        raise ValueError("At most 10 establishment sources are allowed")
    is_url = bool(singular_urls or plural_urls)
    return [(str(value), is_url) for value in values]
...
def _validate_manifest_urls(
    *,
    ...
    establishment_sources: list[tuple[str, bool]],
    company_sources: list[tuple[str, bool]],
    simples_sources: list[tuple[str, bool]],
) -> CnpjDataset | None:
    requested = {
        CnpjDatasetFile.Kind.ESTABLISHMENTS: [
            value for value, is_url in establishment_sources if is_url
        ],
...
def execute_discover_cnpj(job: Job) -> None:
    ...
    establishment_sources = _establishment_sources(payload)
    ...
    if mode == "PREVIEW" and len(establishment_sources) != 1:
        raise ValueError("PREVIEW requires exactly one establishment source")
    if mode == "PREVIEW" and (company_sources or simples_sources):
        raise ValueError("CNPJ complements are allowed only in FULL mode")
    all_sources = (*establishment_sources, *company_sources, *simples_sources)
    remote_requested = any(is_url for _, is_url in all_sources)
    if (
        mode == "FULL"
        and remote_requested
        and not settings.CNPJ_FULL_ENABLED
    ):
        raise ValueError("Remote FULL CNPJ processing is disabled by the kill switch")
    coverage_complete = payload.get("coverage_complete") is True
    if coverage_complete:
        expected_parts = int(payload.get("expected_establishment_parts", 10))
        if mode != "FULL" or expected_parts < 1 or expected_parts > 10:
            raise ValueError("Complete coverage requires FULL mode and a valid part count")
        if len(establishment_sources) != expected_parts:
            raise ValueError("Complete coverage requires every expected establishment part")
    manifest_dataset = _validate_manifest_urls(
        source=source,
        dataset_reference=dataset_reference,
        establishment_sources=establishment_sources,
        company_sources=company_sources,
        simples_sources=simples_sources,
    )
    if coverage_complete and remote_requested:
        if not all(is_url for _, is_url in all_sources):
            raise ValueError("Complete remote coverage cannot mix paths and URLs")
        expected_by_kind = {
            CnpjDatasetFile.Kind.ESTABLISHMENTS: establishment_sources,
            CnpjDatasetFile.Kind.COMPANIES: company_sources,
            CnpjDatasetFile.Kind.SIMPLES: simples_sources,
        }
        for kind, requested_sources in expected_by_kind.items():
            allowed = set(
                manifest_dataset.files.filter(kind=kind).values_list("url", flat=True)
            )
            requested = [value for value, is_url in requested_sources if is_url]
            if len(requested) != len(set(requested)) or set(requested) != allowed:
                raise ValueError(f"Complete coverage requires every manifest {kind} file")
    ...
    limit_reached = False
    establishment_rows = 0
    establishment_matches = 0
    establishment_invalid = 0
    establishment_sources_processed = 0
    active_reader = None
    try:
        for index, (value, is_url) in enumerate(establishment_sources):
            if establishment_matches >= max_results:
                limit_reached = True
                break
            with _source_context(
                ...
                is_url,
                str(job.id),
                f"establishments-{index}",
            ) as source_path:
                establishment_sources_processed += 1
                active_reader = CnpjEstablishmentReader(source_path, filters)
                for establishment in active_reader:
                    ...
                    if establishment_matches + active_reader.stats.rows_matched >= max_results:
                        limit_reached = True
                        break
                establishment_rows += active_reader.stats.rows_read
                establishment_matches += active_reader.stats.rows_matched
                establishment_invalid += active_reader.stats.rows_invalid
                active_reader = None

        invalid_rows = establishment_invalid
        candidates = list(
        ...
        if active_reader is not None:
            ...
            establishment_matches += active_reader.stats.rows_matched
            establishment_invalid += active_reader.stats.rows_invalid
        query_run.records_processed = establishment_rows
        query_run.records_matched = establishment_matches
        query_run.records_failed = establishment_invalid
    ...
    query_run.coverage = {
        ...
        "establishments": {
            "sources_processed": establishment_sources_processed,
            "sources_expected": len(establishment_sources),
            "rows_scanned": establishment_rows,
        },
        "companies": {
    ...
    query_run.finished_at = timezone.now()
    query_run.save()
    if coverage_complete and query_run.status == QueryRun.Status.SUCCEEDED:
    ...
    Job.objects.filter(pk=job.pk).update(
        ...
        records_success=establishment_matches,
